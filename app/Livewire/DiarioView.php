...
use App\Models\Mes;
use App\Exports\DiariohMesExport;
use App\Exports\DiarioxMesExport;
...
class DiarioView extends Component
{
    ...
    public function procesarDiario()
    {
        try {
            $xsql = $this->tipoMes == 'x Mes'
                ? "CALL LdiarioxMes( '$this->mes','$this->empresaId')"
                : "CALL LdiarioHastaMes('$this->mes','$this->empresaId')";
    ...
    public function exportarDiario()
    {
        try {
            ...
            $exportClass = $this->tipoMes == 'x Mes' ? DiarioxMesExport::class : DiariohMesExport::class;

            return Excel::download(new $exportClass($this->result), $this->tipoMes == 'x Mes' ? 'DiarioxMes.xlsx' : 'DiarioHastaMes.xlsx');
        } catch (\Exception $e) {
